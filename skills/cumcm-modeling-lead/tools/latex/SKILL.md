---
name: cumcm-latex
description: 从官方或内置模板创建、编译和校验数学建模 LaTeX 论文项目。
---
...
仅生成 LaTeX/PDF，不检查文档转换工具。诊断会检查 `latexmk`、所选引擎、BibTeX/Biber、`pypdf`、`pdfimages`、`pdftoppm`；任一论文必需项缺失都先报告阻塞，不要写完整论文后才发现无法编译或校验。
...
- 需要在正文交叉引用的关键行间公式使用 `equation` 或 `align` 编号；短小且无需引用的行间公式可以使用无编号环境。
- **禁止** `\[...\]`、`equation*`、`align*`、`gather*`、手写 `\tag`——这些会产生无编号或乱编号公式。
- 编号公式配 `\label{eq:...}`，正文用 `\eqref{eq:...}` 引用；关键符号首次出现时解释含义和单位。
- 多行推导中不需要编号的行加 `\nonumber`；只在最后一行留编号。
- 写完正文后，公式编号必须连续且与正文 `（n）` 引用一致；`paper_check.py` 会核验。
...
CUMCM 的字数、页数、公式和表格只是可覆盖的质量目标，不为满足数量生成无论证价值的内容。每个子问题至少有一幅正式结果图仅在题目和证据需要时采用；页数上限等官方硬约束必须从目标届次规则读取后通过参数传入。
...
论文只交付 `PROJECT_ROOT/完整论文-LaTeX/`、由该项目真实编译得到的 `PROJECT_ROOT/完整论文.pdf` 及同名构建清单。源码项目必须包含模板依赖、参考文献、图表和必要的资源绑定；不得用未编译或哈希不匹配的旧 PDF 代替当前构建结果。
