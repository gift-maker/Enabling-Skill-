...
本技能内置 `tools/drawio/`（drawio-skill v2.1.0，MIT 许可），并内置 `tools/drawio/scibox-diagram/`（sci-box 四套论文级模板）。凡论文需要**技术路线图、子问题求解流程图、数据处理流程图、模型结构图、指标体系图、模型选择决策树**等非数据图，统一走这条工具链，不再手写零散的 `.drawio` XML。

**优先用模板**：问题分析的技术路线图、研究内容全景图、分阶段流程图、任务流水线图，先看 `references/绘图参考/scibox_templates.md` 与 `tools/drawio/scibox-diagram/SKILL.md`，命中四套模板（`roadmap-5band` / `framework-3col` / `stageflow-3col` / `taskflow-land`）就用模板渲染，再用本文件与 drawio-skill 做校验、导出与微调；不命中才走通用手写路径。
...
| 图 | 论文落位 | 说明 |
| --- | --- | --- |
| 技术路线图 `fig_roadmap` | 第 8 部分 问题分析 | 优先 scibox `roadmap-5band`（提出问题 → 数据与指标 → 方法与机制 → 结果对比 → 评价推广）；图后紧跟分析 |
| 研究内容全景图 `fig_framework` | 第 8 部分 问题分析 / 模型总览 | scibox `framework-3col`（左阶段 / 中内容 / 右方法三向对应）；按需 |
| 分阶段流程图 `fig_flow_qN` | 第 3 部分 该问"模型的求解" | 复杂执行流程（判断分支/成果分发）优先 scibox `stageflow-3col`；简单输入→算法→输出用通用手写；图后紧跟分析 |
| 任务流水线图 `fig_taskflow` | 多问任务拆解 / 答辩 | scibox `taskflow-land`（横版，任务块内流水线 + 做法细节）；按需 |
| 数据处理流程图 `fig_pipeline` | 第 2 部分 数据预处理 | 清洗 → 特征构造 → 划分；阶段多时可用 scibox `stageflow-3col`；图后紧跟分析 |
...
**不用 drawio：** 数据图（折线/柱状/散点/热力/箱线等）仍走 `scripts/plot_style.py` + `references/绘图参考/chart_knowledge_base.md`；需要公式节点、精确数学连线或 2D 几何的概念图优先 TikZ；节点极多且由数据驱动布局的网络图用 Matplotlib/networkx。

## 执行前必读（渐进式加载）

1. 本文件（国赛规则与落位）
2. `references/绘图参考/scibox_templates.md`（国赛落位与四套模板选择）→ 命中模板时读 `tools/drawio/scibox-diagram/SKILL.md` 与对应 `references/<template>.md`
3. `tools/drawio/SKILL.md`（完整工作流：依赖检查 → 规划 → 生成 → 导出 → 自检 → 审核循环 → 定稿）
4. 按图类型读 `tools/drawio/references/diagram-types.md`（流程图/ML/架构等预设）
5. 手写 XML 前读 `tools/drawio/references/xml-authoring.md`（文件骨架、形状/边写法、配色、网格）
6. 需要样式预设时读 `tools/drawio/references/style-presets.md`，内置预设见 `tools/drawio/styles/built-in/`
7. 导出失败或视觉自检不通过时读 `tools/drawio/references/troubleshooting.md`
