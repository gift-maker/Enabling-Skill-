...
- 每个递进子问题都与上一问比较并重新识别结构，第三问优先检查；新增离散决策、组合约束、资源配置、时序、风险或不确定性时，重新判断是否已由统计/预测升级为决策/组合优化或二者串联，不把第三问机械判为 NP-hard。
...
- DrawIO 类非数据图统一使用本技能内置的 `tools/drawio/`（drawio-skill v2.1.0）并按 `references/绘图参考/drawio_flowchart.md` 的硬性规则执行（图内无标题、图题在图下、图后跟分析、同类型 ≤3 张、色盲安全配色、节点文字 ≥10px）；不手写零散 `.drawio` XML。
...
- 速查：需要公式或精确连线时优先 TikZ；需要可编辑流程结构时优先 DrawIO（走 `tools/drawio/`）；需要真实场景或复杂 3D 视觉时才考虑生成式图片。
...
- 启发式算法（GA/SA/PSO）应固定随机种子，并按结论风险采用足够的多起点、多种子、小规模精确解或上下界证据。重复次数由稳定性和计算预算决定；新增运行预计不改变模型选择、主要结论、分组、排名或最终决策时停止追加。
...
- **蒙特卡洛**：按目标统计量的标准误、置信区间或分位数稳定性确定采样量，并预设最大预算；报告区间而不只报均值，不把固定的 10,000 次当作普遍门槛。
...
- 中国赛摘要（含标题和关键词）原则上不超过一页，不需要翻译成英文；页码从摘要页"1"开始、位于页脚中部。
...
2. **问题重述**：不是照抄题面，而是建模者的理解和重新表达
3. **问题分析**：整体思路、技术路线、子问题依赖，成稿时紧跟问题重述
4. **模型假设**：每条假设必须有必要性说明和在正文中的引用，不能是空洞罗列
5. **符号说明**：所有后文用到的符号在此定义，有量纲标注
6. **模型建立与求解**：每个子问题独立成节；先说明建模思路，再给出数学表达，再说明求解方法
7. **结果分析与检验**：数值结果 + 约束验证 + 误差分析
8. **灵敏度分析**：关键参数扰动时结果如何变化，说明模型鲁棒性
9. **模型评价与推广**：优缺点、改进方向、推广场景
10. **参考文献**：只引用真实存在的文献，格式规范
11. **附录**：支撑材料文件列表 + 完整可运行源程序（官方要求，见国赛规范）

### 灵敏度分析规范

- 对可能改变主要结论的关键参数做与题型匹配的灵敏度或稳健性分析；若已有合适验证且新增扰动不会改变模型选择、主要结论、分组、排名或最终决策，停止追加并记入局限或附录。
