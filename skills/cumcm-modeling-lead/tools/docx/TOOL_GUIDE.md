...
公式必须是真实数学对象（原生 OMML）：用 `pf.equation(doc, r"...")` 或 `equations.py` 写入。**禁止**在正文段落文本里写 `x\_i`、`$x_i$` 等 LaTeX/Markdown 转义形式——下标注记在 LaTeX 输入里写 `x_i`（不转义），输出为真实上下标。`verify-formulas` 会检查正文是否残留 `\_`、`$` 等转义文本，出现即 FAIL。
...
python scripts/equations.py verify-formulas "<PROJECT_ROOT>/完整论文.docx"
```

`verify-conversion` 仅适用于由本工具的 Pandoc 转换生成、带 `.conversion.json` 的 DOCX；直接由 `paper_format.py` 构建时省略。**`verify-formulas` 是最终公式输出验收**：检查 `EQ_` 占位符是否全部替换、统计 OMML 公式数量、校验 `document.xml` 结构，任一公式占位符残留即返回非零退出码。`paper_format.py validate` 输出结构化指标，并在官方前置结构、篇幅质量目标、公式/图/表数量、公式占位符残留、图表编号与正文引用、参考文献双向对应或实际页数任一不满足时返回非零退出码。所有竞赛默认至少 8 幅图；CUMCM 默认的 15000 字词单位和约 20 页只是质量目标。以 2026 年官方规范为例，正文不超过 30 页才是硬约束。只有当届官方规则或用户明确要求允许偏离时才能调整目标并记录依据。结构校验后，把 DOCX 渲染成 PDF 或图片抽检分页、公式、表格、图片、页眉页脚和字体替换。
