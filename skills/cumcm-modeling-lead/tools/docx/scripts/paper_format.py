...
def _add_tab_run(p):
    """Append an explicit <w:tab/> run (tab stops render correctly)."""
    run = p.add_run()
    run._element.append(OxmlElement("w:tab"))
    return run


def _equation_count(doc):
    """Count equations already present in the document body."""
    ns = {"m": "http://schemas.openxmlformats.org/officeDocument/2006/math"}
    return len(doc.element.body.findall(".//m:oMath", ns))


def equation(doc, latex, number=None):
    """Insert a display equation: formula centered, number right-aligned.

    国赛规范：公式独占一行、居中；(n) 编号右对齐、连续编号。
    实现：段落设置「中制表位(版心中点) + 右制表位(版心右缘)」，
    公式用行内 m:oMath 放在两个制表位之间，编号默认自动递增。
    """
    # 制表位位置相对左边距定位（Word 约定）：中制表位=版心一半，右制表位=版心宽度
    section = doc.sections[0]
    usable = section.page_width - section.left_margin - section.right_margin
    center_pos = usable // 2
    right_pos = usable

    p = paragraph(doc, align=WD_ALIGN_PARAGRAPH.LEFT)
    pf = p.paragraph_format
    pf.tab_stops.add_tab_stop(center_pos, WD_TAB_ALIGNMENT.CENTER)
    pf.tab_stops.add_tab_stop(right_pos, WD_TAB_ALIGNMENT.RIGHT)
    _add_tab_run(p)
    ...
    _add_tab_run(p)
    if number is None:
        number = _equation_count(doc)
    set_run_font(p.add_run(f"({number})"), size=12)
    return p
...
def _numbered_object_issues(doc, kind, object_count):
    ...
    for paragraph in doc.paragraphs:
        ...
        if caption:
            ...
            caption_order.append(number)
            captions[number] = text
        else:
            body_references.update(int(number) for number in reference_pattern.findall(text))

    issues = []
    duplicates = sorted({number for number in caption_order if caption_order.count(number) > 1})
    if duplicates:
        issues.append(f"{kind}题注编号重复: {duplicates}")
    if caption_order != sorted(caption_order):
        issues.append(f"{kind}题注顺序错乱（题注编号必须按正文出现顺序递增）")
...
def validate_paper_structure(
    ...
    *,
    ...
    leftover_eq = re.findall(r"\bEQ_[0-9A-F]{8}\b", all_text)
    if leftover_eq:
        errors.append(
            f"存在未替换的公式占位符（{len(leftover_eq)} 处，如 {leftover_eq[0]}），"
            "公式未正确输出，必须全部替换为 OMML"
        )
