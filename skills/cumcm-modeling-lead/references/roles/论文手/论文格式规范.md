...
- **禁止转义文本**：正文与公式中不得出现 `x\_i`、`$x_i$` 这类 Markdown/LaTeX 转义写法；下标/上标必须是真实数学结构（Word 原生 OMML 上下标，或 LaTeX 数学模式的 `x_i`/`x^2`）。`x\_i` 说明公式被当普通文本写入，`equations.py verify-formulas` 与 `scripts/paper_check.py` 会判 FAIL。
