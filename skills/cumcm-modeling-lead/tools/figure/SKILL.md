...
先确认模型与结果状态，再选择路线。模型冻结后，图表交付是论文包的强制门，不是可选美化：先读取 `../../references/终稿图表交付协议.md` 并填写当前问图表任务卡。
...
7. 保存源数据、脚本、PNG/SVG/PDF、图注、审计结果和图表—结论映射。

没有数据剖析、图表契约、自动审计和人工视觉回审的 PNG 只能视为草图，不能进入终稿图表目录。

主动拦截掩盖小样本分布的均值柱状图、双 Y 轴、装饰性 3D、饼图、不合理截断、rainbow/jet 色图、分类点错误连线和图例遮挡。

## Nature 终稿增强

每问必须从已冻结图中指定一张核心图；只有模型与结果已冻结、图表结论和源数据已锁定时，这张核心图才进入 Nature 路线：
...
终稿图完成必须满足：数值与冻结结果一致、图型支持声明的结论、无裁切遮挡、最终尺寸可读、源数据与脚本可追溯，并经过人工查看。任一问题未完成“结果图 + 决策解释图 + 一张 Nature 核心图”的最低组合时，项目不得标为论文可交付。
