...
def verify_formulas(output_path):
    """最终公式输出验收：无占位符残留、OMML 数量报告、document.xml 结构有效。"""
    from docx import Document

    issues = []
    path = Path(output_path).resolve()
    if not path.is_file() or path.suffix.casefold() != ".docx":
        issues.append(f"DOCX 输出不存在：{path}")
        return {
    ...
    equations = len(doc.element.body.findall(".//m:oMath", ns))

    leftovers = []
    for para in doc.paragraphs:
        leftovers += re.findall(r"EQ_[0-9A-F]{8}\b", para.text)
    for table in doc.tables:
        for row in table.rows:
            for cell in row.cells:
                leftovers += re.findall(r"EQ_[0-9A-F]{8}\b", cell.text)
    leftovers = sorted(set(leftovers))
    if leftovers:
        issues.append(f"存在未替换的公式占位符（{len(leftovers)} 处）：{leftovers[:10]}")

    # 公式转义文本泄漏：正文不得出现 x\_i / $...$ 等 Markdown/LaTeX 转义记号，
    # 公式必须是原生 OMML（paper_format.equation / equations.py 写入），不是转义文本。
    escape_samples: list[str] = []
    text_units: list[str] = [p.text for p in doc.paragraphs]
    for table in doc.tables:
        for row in table.rows:
            for cell in row.cells:
                text_units.append(cell.text)
    for unit in text_units:
        for m in re.finditer(r"\\_|\$", unit):
            ctx = unit[max(0, m.start() - 18):m.end() + 18].replace("\n", " ")
            if ctx not in escape_samples:
                escape_samples.append(ctx)
    if escape_samples:
        issues.append(
            f"正文存在公式转义文本（{len(escape_samples)} 处，如 x\\_i / $...$）："
            f"{escape_samples[:8]}；公式必须写为原生 OMML"
            "（paper_format.equation / equations.py），禁止把 LaTeX/Markdown 转义当正文文本"
        )
    ...
    try:
        ...
        issues.append(f"document.xml 结构无效：{error}")
...
def build_parser():
    ...
    vff = sub.add_parser("verify-formulas", help="最终公式输出验收：无占位符残留 + OMML 数量 + XML 结构")
    vff.add_argument("input", help="待验收的 .docx 文件")
...
def main():
    ...
    elif args.mode == "verify-formulas":
        result = verify_formulas(args.input)
        print(json.dumps(result, ensure_ascii=False, indent=2))
        if not result["passed"]:
            sys.exit(1)
