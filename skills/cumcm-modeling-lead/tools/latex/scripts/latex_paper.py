...
def _display_equations(source: str, issues: list[str]) -> int:
    ...
    starred = len(re.findall(
        r"\\begin\s*\{(?:equation|align|alignat|gather|multline|eqnarray)\*\}", source
    ))
    ...
    if opens:
        issues.append(
            r"存在未编号行间公式 \[...\]，公式必须用 equation/align 编号环境（编号自动右对齐）"
        )
    if starred:
        issues.append(
            "存在星号无编号公式环境（equation*/align* 等），必须改用编号环境，编号右对齐"
        )
...
def inspect_paper(
    ...
    *,
    ...
    if aux_path.is_file():
        ...
        src_eq_labels = len(re.findall(r"\\label\s*\{eq:", source))
        aux_eq_labels = len(re.findall(r"\\newlabel\s*\{eq:", aux_text))
        if src_eq_labels and src_eq_labels != aux_eq_labels:
            issues.append(
                f"公式标签数量不一致：源码 {src_eq_labels} 个 eq: 标签，"
                f".aux 仅记录 {aux_eq_labels} 个（公式可能未正确编译输出）"
            )
        for kind, prefix in (("图", "fig"), ("表", "tab")):
            numbers = [
                int(number)
                for number in re.findall(
                    rf"\\newlabel\{{{prefix}:[^}}]*\}}\{{{{(\d+)}}", aux_text
                )
            ]
            if not numbers:
                continue
            ...
            if missing:
                issues.append(f"{kind}号不连续，缺少: {missing}")
            if duplicates:
                issues.append(f"{kind}号重复: {duplicates}")
